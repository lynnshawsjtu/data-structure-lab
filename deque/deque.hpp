...
public:
    ...
    public:
         ...
         * as well as operator-
         */
        iterator (const deque *q=NULL,Node *n=NULL,int p=0):que(q),node(n),pos(p){}
        iterator(const iterator &rhs):que(rhs.que),node(rhs.node),pos(rhs.pos){}
        ...
        iterator operator+(const int &n) const {
            ...
            if(n<0){
                int dis=-n;
                while(ans.node->prev&&ans.pos<dis){
                    dis-=ans.pos+1;
                    ans.node=ans.node->prev;
                    ans.pos=ans.node->inside-1;
                }
                if(ans.pos<dis&&ans.node->prev==NULL) throw index_out_of_bound();
                ans.pos-=dis;
            } else if(n>0){
                int dis=n;
                while(ans.node->next!=ans.que->tail&&(ans.pos>=ans.node->inside-dis)){
                    dis-=ans.node->inside-ans.pos;
                    ans.pos=0;
                    ans.node=ans.node->next;
                }
                if(ans.node->next==NULL) throw index_out_of_bound();
                ans.pos+=dis;
            }
            return ans;
        }
        ...
        iterator operator++(int) {
            ...
            if(pos==node->inside-1){
                pos=0;
                if(node->next==NULL) throw index_out_of_bound();
                else node=node->next;
            }
            else pos++;
            return ans;
        ...
        iterator operator--(int) {
            iterator ans(*this);
            if(pos==0){
                if(!node->prev) throw index_out_of_bound();
                else{
                    node=node->prev;
                    pos=node->inside-1;
                }
        ...
        bool operator==(const iterator &rhs) const {
            return (node==rhs.node&&pos==rhs.pos)||(node->next&&rhs.node->next==node&&rhs.pos==rhs.node->inside)
            ||(rhs.node->next&&node->next==rhs.node&&pos==node->inside);
        }
    ...
    class Node{
        ...
        Node *next;
        public:
        Node(int i=Size):chunkSize(i),inside(0),next(NULL),prev(NULL),data(NULL){
        ...
        ~Node(){clear();}
        Node &oparator(const Node &rhs){
            if(&rhs!=this){
                clear();
                chunkSize=this.chunkSize;
                inside=this.inside;
                data=new T*[chunkSize];
                for(int i=0;i<rhs.inside;++i) data[i]=new T(*rhs.data[i]);
            }
            return *this;
        }
        void clear(){
        ...
        T &at(int index){
            if(index<0||index>=inside) throw index_out_of_bound();
            return *(data[index]);
        }
        ...
        void insert(int index,const T& value){
            ...
            for(int i=inside;i>index;--i) data[i]=data[i-1];
        ...
        void erase(int index){
            delete data[index];
            for(int i=index;i<inside-1;++i) data[i]=data[i+1];
            inside--;
        }
    };
    /**
